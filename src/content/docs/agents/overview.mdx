...
> **Note:** `execShell` is disabled by default for security. Enable it by setting `ALLOW_SHELL_EXEC=true` in the agent's environment.
...
Set the model with the `AGENT_MODEL` environment variable (e.g. `claude-sonnet-4-5`, `gemini-2.0-flash`).
