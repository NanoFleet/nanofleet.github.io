...
When an agent pack lists keys in `requiredEnvVars`, NanoFleet automatically resolves them from the vault and injects them as environment variables at deploy time. You don't need to manually configure each agent.

The same applies to plugins: if a plugin manifest lists `envVars`, those are resolved from the vault during installation.
