...
Running NanoFleet on a remote server requires HTTPS — browsers block certain features over plain HTTP, and exposing port 8080 without TLS is not recommended.

Two common approaches:

### Option 1 — Domain + reverse proxy (Traefik)

NanoFleet ships with a Traefik configuration that handles TLS automatically via Let's Encrypt.

```bash
cp docker-compose.prod.yml docker-compose.override.yml
```

Edit `docker-compose.override.yml` and set your domain:
...
Point your DNS A record to the server's public IP, then restart:

```bash
docker compose up -d
```

Traefik obtains a certificate automatically on first startup.

### Option 2 — SSH tunnel (no domain needed)
